...
//            View mostVisibleView = outMostVisibleItem.getView();
    ...
    public ListItemData getCurrent() {
        return mCurrentItem;
    }

    public void setFirstItem(ItemsPositionGetter itemsPositionGetter) {
        mCurrentItem.fillWithData(0, itemsPositionGetter.getChildAt(0));
        activeCurrentItem(mCurrentItem);
    }

    public boolean resume() {
        if (mCurrentItem != null && mCurrentItem.isAvailable()) {
            activeCurrentItem(mCurrentItem);
            return true;
        }
        return false;
